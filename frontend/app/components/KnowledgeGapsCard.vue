  ...
  >
    ...
    <div v-if="knowledgeGaps.prerequisites && knowledgeGaps.prerequisites.length > 0" class="mb-4">
      <div class="flex items-center gap-1 mb-2">
        ...
        <span class="text-xs font-medium text-gray-600">前置知识</span>
      </div>
      <div class="flex flex-wrap gap-2">
        <span
          v-for="(prereq, index) in knowledgeGaps.prerequisites"
          :key="index"
          class="px-3 py-1.5 bg-indigo-50 border border-indigo-200 text-indigo-700 text-xs rounded-lg hover:bg-indigo-100 transition-colors cursor-pointer"
        >
          {{ prereq }}
        </span>
      </div>
      <p class="text-xs text-gray-600 mt-2">阅读本文建议先了解以上概念</p>
    </div>
    ...
    <div v-if="knowledgeGaps.assumptions && knowledgeGaps.assumptions.length > 0" class="mb-4">
      <div class="flex items-center gap-1 mb-2">
        ...
        <span class="text-xs font-medium text-gray-600">隐含假设</span>
      </div>
      <ul class="space-y-1.5">
        <li
          v-for="(assumption, index) in knowledgeGaps.assumptions"
          :key="index"
          class="text-xs text-gray-700 flex items-start gap-2 p-2 bg-blue-50/50 rounded-lg border border-blue-200/50"
        >
          <span class="text-blue-600 mt-0.5 flex-shrink-0">→</span>
          <span>{{ assumption }}</span>
        </li>
      </ul>
      <p class="text-xs text-gray-600 mt-2">作者未明说但默认读者知道的前提</p>
    </div>
    ...
    <div v-if="knowledgeGaps.missing_context && knowledgeGaps.missing_context.length > 0" class="mb-4">
      <div class="flex items-center gap-1 mb-2">
        ...
        <span class="text-xs font-medium text-gray-600">缺失背景</span>
      </div>
      <ul class="space-y-1.5">
        <li
      ...
      <p class="text-xs text-gray-600 mt-2">文中未提及但可能重要的上下文信息</p>
    </div>
    ...
    <div v-if="knowledgeGaps.related_concepts && knowledgeGaps.related_concepts.length > 0" class="mb-4">
      <div class="flex items-center gap-1 mb-2">
        ...
        <span class="text-xs font-medium text-gray-600">拓展阅读</span>
      </div>
      <div class="flex flex-wrap gap-2">
        <span
          v-for="(concept, index) in knowledgeGaps.related_concepts"
          :key="index"
          class="px-3 py-1.5 bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs rounded-lg hover:bg-emerald-100 transition-colors cursor-pointer"
        >
          {{ concept }}
        </span>
      </div>
      <p class="text-xs text-gray-600 mt-2">推荐继续学习的相关主题</p>
    </div>
